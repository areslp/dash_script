...
def start(server=None, name=None):
    external_stylesheets = ['https://codepen.io/chriddyp/pen/bWLwgP.css']
    abs = pd.read_csv('data/absolute_error.csv')  # df
    print(abs.head())
    ...
    seq = pd.read_csv('data/sequence_error.csv')  # df2
    print(seq.head())
    print(len(seq.index))

    traj = pd.read_csv('data/trajectories.csv')  # df3
    print(traj.head())
    print(len(traj.index))

    df4 = pd.read_csv('data/sequence_covariance_error.csv')  # df4
    print(df4.head())
    print(len(df4))
    ...
    app = dash.Dash(__name__, server=server, routes_pathname_prefix=name,
